...
Describe "Template File Checks" { 
    ...
    Context "Component contents" { 
        It "Has a JSON template" { 
             "$TemplatePath\recoveryServicesVault.json" | Should -Exist 
        }
    ...
    Context "Template files contents" { 
        It "Has the expected properties in the json" {
            ...
            $TemplateProperties = (get-content "$TemplatePath\recoveryServicesVault.json" | ConvertFrom-Json -ErrorAction SilentlyContinue) | Get-Member -MemberType NoteProperty | ForEach-Object Name
        ...
        It "Creates the expected Azure resources" { 
            ...
            $TemplateResources = (get-content "$TemplatePath\recoveryServicesVault.json" | ConvertFrom-Json -ErrorAction SilentlyContinue).Resources.type 
            Assert-Equivalent -Actual $TemplateResources -Expected $ExpectedResources
        }

    }
    Context "Template Validation" { 
        It "Template $TemplatePath\recoveryServicesVault.json and parameter file passes validation" { 
            ...
            $ValidationResult = Test-AzResourceGroupDeployment -ResourceGroupName $TempValidationRG -Mode Complete -TemplateFile "$TemplatePath\recoveryServicesVault.json" -TemplateParameterFile "$TemplatePath\test.params.json" 
