<#
    .SYNOPSIS
        Create Remediation task for the resources that are not-compliant with the Azure security Center export settings.

    .DESCRIPTION
        This runbook loops through all subscriptions in the Customer tenant and on each subscription, 
        loops through the policies that are not-compliant with the policy definition for enabling ASC export to Log Analytics, removes existing remediation task (if present)
...
$policyDefinitionName = "azdefenderexport-change-policy-def-assignment"
