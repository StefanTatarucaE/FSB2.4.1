<#
    .SYNOPSIS
        Create Remediation task for the resources that are not-compliant with the Kubernetes Initiative

    .DESCRIPTION
        This runbook loops through all subscriptions in the Customer tenant and on each subscription, 
        loops through the policies that are not-compliant with the policy definition for the Kubernetes Initiative, removes existing remediation task (if present)
...
$policyDefinitionName = "aks-change-policy"
...
# Determine which automation account initiated this runbook
...
foreach ($subscription in $subscriptions) {
    ...
    $nonCompliantPolicies = Get-AzPolicyState -DefaultProfile $Subcontext | Where-Object { $_.ComplianceState -eq "NonCompliant" -and $_.PolicyDefinitionAction -eq "deployIfNotExists" -and $_.PolicyAssignmentName -like "*-" + $policyDefinitionName + "-*" }
