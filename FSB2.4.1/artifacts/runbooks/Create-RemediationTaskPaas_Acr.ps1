<#
    .SYNOPSIS
        Create Remediation task for the resources that are not-compliant with the containerRegistry PolicyInitiative

    .DESCRIPTION
        This runbook loops through all subscriptions in the Customer tenant and on each subscription, 
        loops through the policies that are not-compliant with the policy definition for the containerRegistryPolicy Initiative, removes existing remediation task (if present)
...
$policyDefinitionName = "acr-change-policy"
