...
# kql Query to retrieve all <CompanyName>Managed: True tagged resources.
$kqlQuery = @"
resources
| where tolower(tostring(tags)) contains '"${tagPrefix}managed":"true"'
| join kind=leftouter (resourcecontainers
| where type == "microsoft.resources/subscriptions"
| project subscriptionId, subname=name) on subscriptionId
| project["Resource Name"]=name, ["Type"]=type, Subscription=subname, ["Resource Group"]=resourceGroup| sort by ["Resource Name"] asc
"@
...
try {
    ...
    #Get <Company>Managed loganalytics workspace in customer subscriptions
    ...
    # create Resources with <compnay>Managed tag report (Azure resource Graph)
    # determine previous month for report title and name 
    # report is created on first day of (next) month
    If ($DayOfMonth -eq $monthreportday) {
        ...
        $Title = -join ("Resources with ${tagPrefix}Managed tag is ""true"" monthly Report ", $date)
        ...
        #Format HTML report
        If ($report.count -eq 0) { $Title = -join ($($Title), "<br><br><font color=""red""> - No resources with ${tagPrefix}Managed tag set to ""true"" found! -</font>") }
        ...
        $CSVname = "CSV Reports - " + $date + $manual + "_Resources_${tagPrefix}Managed_tag_Report.csv"
        $HTMLname = "HTML Reports - " + $date + $manual + "_Resources_${tagPrefix}Managed_tag_Report.html"
